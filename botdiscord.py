...
async def on_message(message):
    ...
    # --- Nuevo comando $dado <caras> ---
    elif message.content.startswith('$dado'):
        parts = message.content.split()
        if len(parts) == 2 and parts[1].isdigit():
            caras = int(parts[1])
        else:
            caras = 6  # Valor por defecto

        numero = random.randint(1, caras)
        await message.channel.send(f"🎲 Has sacado un **{numero}** en un dado de {caras} caras.")

    else:
        await message.channel.send("Comando no reconocido. Usa `$hello`, `$bye`, `$pass <n>`, `$meme <PALABRA>` o `$dado <caras>`")
